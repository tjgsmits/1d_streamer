...
for i, df in enumerate(data):
    ...
    # Plot nion vs x
    axes[0, 0].plot(df["x"], df["nion"], color=color, marker='o', label=f'File {i+1}')
    ...
    # Plot phi vs x
    axes[0, 1].plot(df["x"], df["phi"], color=color, marker='o', label=f'File {i+1}')
    ...
    # Plot n vs x
    axes[1, 0].plot(df["x"], df["n"], color=color, marker='o', label=f'File {i+1}')
    ...
    # Plot E vs x
    axes[1, 1].plot(df["x"], df["E"], color=color, marker='o', label=f'File {i+1}')
