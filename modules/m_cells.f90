module m_cells
    ...
    contains
    subroutine boundary_conditions(delta_x, Ngrid, BC_type, bc_value1, bc_value2, u)
        real, intent(in)             :: delta_x
        integer, intent(in)          :: Ngrid
        character(len=*), intent(in) :: BC_type
        real, intent(in)             :: bc_value1, bc_value2
        real, intent(inout)          :: u(Ngrid+4)

        ! Important to note is that the BC are applied at the ghost cells
        select case(BC_type)
        case('neumann')
                u(2)         = u(3) - bc_value1 * delta_x
                u(1)         = u(3) - bc_value1 * delta_x
                u(size(u)-1) = u(size(u)-2) + bc_value2 * delta_x
                u(size(u))   = u(size(u)-2) + bc_value2 * delta_x
        case('dirichlet')
                u(2)         = bc_value1    
                u(1)         = bc_value1
    ...
    subroutine ghost_cell(Ngrid, delta_x, BC_type, bc_value1, bc_value2, input_value, ghost_layer)
        ...
        call boundary_conditions(delta_x, Ngrid, BC_type, bc_value1, bc_value2, ghost_layer)
